...
def render_note(note: WikiNote) -> str:
    ...
    if note.metadata:
        lines.append(
            f"- generated_at: {note.metadata.generated_at_iso()}"
        )
        lines.append(f"- source_policy: {note.metadata.source_policy}")
        lines.append(
            f"- validation_status: {note.metadata.validation_status}"
        )
        lines.append(f"- evidence_count: {note.metadata.evidence_count}")
        lines.append(
            f"- generator_version: {note.metadata.generator_version}"
        )
        lines.append(f"- schema_version: {note.metadata.schema_version}")
        if note.metadata.source_documents:
            lines.append("- source_documents:")
            for sd in note.metadata.source_documents:
                lines.append(f"  - document_id: {sd.document_id}")
        if note.metadata.source_chunks:
            lines.append("- source_chunks:")
            for sc in note.metadata.source_chunks:
                lines.append(f"  - chunk_id: {sc.chunk_id}")
    lines.append("")
...
def _render_frontmatter(note: WikiNote, lines: list[str]) -> None:
    ...
    meta = note.metadata
    if meta:
        lines.append(f"note_id: {meta.note_id}")
        lines.append(f"note_type: {meta.note_type}")
        lines.append(f"status: {meta.status}")
        lines.append(f"generated_at: {meta.generated_at_iso()}")
        lines.append(f"generated_by: {meta.generated_by}")
        lines.append(f"generator_version: {meta.generator_version}")
        lines.append(f"schema_version: {meta.schema_version}")
        lines.append(f"source_policy: {meta.source_policy}")
        ...
        if meta.source_documents:
            ...
            for sd in meta.source_documents:
                lines.append(f"  - document_id: {sd.document_id}")
                if sd.source_path:
                    lines.append(f"    source_path: {sd.source_path}")
                if sd.source_raw_hash:
                    lines.append(f"    source_raw_hash: {sd.source_raw_hash}")
                if sd.content_hash:
                    lines.append(f"    content_hash: {sd.content_hash}")

        if meta.source_chunks:
            lines.append("source_chunks:")
            for sc in meta.source_chunks:
                lines.append(f"  - document_id: {sc.document_id}")
                lines.append(f"    chunk_id: {sc.chunk_id}")
                if sc.source_raw_hash:
                    lines.append(f"    source_raw_hash: {sc.source_raw_hash}")
                if sc.raw_text_hash:
                    lines.append(f"    raw_text_hash: {sc.raw_text_hash}")
                if sc.cleaned_text_hash:
                    lines.append(f"    cleaned_text_hash: {sc.cleaned_text_hash}")
                if sc.evidence_text_hash:
                    lines.append(f"    evidence_text_hash: {sc.evidence_text_hash}")
